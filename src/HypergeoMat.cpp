...
double betaratio(Eigen::ArrayXi kappa, Eigen::ArrayXi mu, int k, double alpha) {
...
R T_(double alpha, T a, T b, Eigen::ArrayXi kappa) {
  ...
  //  Eigen::Array<R> cc(b.size());
  //  cc.fill(c);
  R prod1_den = (b + c).prod();  // productArma<T, R>(b + cc);
  if(prod1_den == 0.0) {
    return 0.0;
  }
  ...
  //  ccc.fill(c);
  R prod1_num = (a + c).prod();  // productArma<T, R>(a + ccc);
